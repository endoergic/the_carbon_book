...
4. Genuinely both, and which one it is depends on momentum that isn't guaranteed. As a virtuous circle: discerning demand (SBTi/VCMI) pays premiums for labeled, durable credits (ICVCM), financing better projects, deepening high-integrity supply, making it easier for more buyers to commit. As a fragile loop: every node is voluntary and mutually dependent, so a stall anywhere propagates — if buyers retreat, the CCP premium collapses, developers stop pursuing the label, and the demand-side rules lose the supply they presuppose. The honest read is a flywheel that hasn't built enough mass to be self-sustaining: held in motion by a small set of committed buyers and still-fragile voluntary institutions, its durability through a political or economic shock the unresolved question. Mandatory disclosure is the likeliest "ratchet" to make the circle robust rather than fragile — which is why the reform's fate ties to the disclosure regimes of Ch. 5 and Ch. 13.
...
| SBTi Corporate Net-Zero Standard V2 | second consultation; final expected mid–late 2026, mandatory for new near-term targets from 1 Jan 2028 | Nov 2025 | SBTi |
