...
2. Why is natural CO₂ extraction from underground domes (e.g., Bravo Dome) for EOR essentially climate-negative? What does the mass balance look like?
...
2. Natural CO₂ from underground domes is geologically sequestered CO₂. Extracting it, using it for EOR, and then having the CO₂ remain in the reservoir means no net addition to the atmosphere — the CO₂ was underground before and remains underground. From a mass balance perspective: -1 ton extracted + 1 ton re-injected = 0 atmospheric addition. The EOR use is essentially carbon-neutral on the molecule, unlike burning the oil it helps extract (which does add to the atmosphere, making EOR-coupled CCS a contested climate claim when the whole system is counted).
