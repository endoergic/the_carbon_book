...
1. The natural gas burned at the refinery is Scope 1 (stationary combustion in a facility the company controls). The grid electricity is Scope 2 (purchased energy). The methane from faulty valves is Scope 1 (fugitive emissions). The emissions when motorists burn the gasoline are Scope 3, Category 11 (use of sold products) — the dominant category for a fuel producer. The trucks delivering crude are Scope 3, Category 4 (upstream transportation and distribution), since they are third-party logistics moving goods into the company rather than company-owned vehicles.

2. Dual reporting exists because the two methods answer different questions and neither alone is complete. The location-based method uses grid-average emission factors for where electricity is consumed, capturing the physical reality of what emissions actually resulted. The market-based method uses contractual instruments (RECs, PPAs, green tariffs) to reflect what the company is doing to drive clean electricity through procurement. Requiring both prevents a company from hiding behind contracts while ignoring its physical grid footprint, and prevents undervaluing genuine procurement effort. Location-based answers "what physically happened"; market-based answers "what is the company doing about it."
...
1. Under PCAF, the bank's financed emissions from these loans equal each borrower's emissions multiplied by an attribution factor — for lending, the outstanding loan amount divided by the borrower's enterprise value (or total debt plus equity). Summed across the $100B portfolio, that gives the bank's financed-emissions inventory. To calculate it, the bank needs each oil and gas borrower's own emissions (ideally Scope 1, 2, and the material Scope 3, especially Category 11 for fuel producers) and each borrower's enterprise value or financing structure to compute the attribution factor. Where borrower data is missing, PCAF allows estimation with a data-quality score.

2. The substantive question is how much responsibility a bank bears for emissions it enabled through capital-markets underwriting but does not hold on its balance sheet, unlike direct lending. Arguments for a higher weighting: the bank facilitated the financing that enabled the emissions, and a low weight lets banks shift fossil financing into underwriting to dodge accountability. Arguments for a lower weighting: underwriting is a one-time service with no ongoing exposure, so even 33% may overstate responsibility relative to a loan held for years. The 33% figure is a negotiated compromise reflecting politics more than science, and is likely to be revisited.

3. Its climate performance is unchanged — the oil company's physical emissions did not move; only its share price (and therefore the enterprise-value denominator in the attribution factor) fell, mechanically raising attributed emissions. This exposes a real weakness: PCAF's enterprise-value attribution measures something that fluctuates with markets rather than with real-world decarbonization, so a bank can look worse in a down market having done nothing wrong (and better in a bull market). It suggests PCAF is partly measuring market valuation rather than climate impact, which is why the 3rd edition added fluctuation guidance.

4. On paper, without changing real-world impact, a bank could: (1) sell or run off loans to high-emitting borrowers so they appear on someone else's books while the same projects keep operating; (2) shift fossil financing from direct lending into capital-markets underwriting, which is weighted at only 33% as facilitated emissions; (3) benefit from rising borrower equity prices, which enlarges enterprise-value denominators and shrinks attributed emissions with no physical change; or (4) reweight the portfolio toward low-emitting sectors or use favorable data-quality estimates and base-year choices, lowering the reported number without reducing financed real-economy emissions.
...
1. The 95% coverage rule requires that reported Scope 3 emissions cover at least 95% of categories 1–15, with any exclusions quantified, disclosed, and justified. It rules out selective Scope 3 truncation — the common pattern of reporting only the convenient categories while quietly omitting large ones (like Category 11 for a fuel producer or Category 1 for a manufacturer). It is significant because Scope 3 is typically 60–90% of a company's total emissions, so partial reporting can materially understate impact while technically complying. The rule forces completeness and makes omissions visible rather than silent.
...
1. The Scope 3 figure is implausibly small. For a consumer products manufacturer, Scope 3 — especially Category 1 (purchased goods and services) and Category 11 (use of sold products) — typically dwarfs Scope 1+2, often representing 60–90% of total emissions. Here Scope 3 is reported as less than half of Scope 1+2, which strongly suggests Scope 3 truncation: the company has likely omitted or under-counted major categories such as the embodied emissions of materials it buys or the use and disposal of its products. This is the classic pattern of reporting the easy scopes in detail while materially understating the largest source.

2. The large gap means the company is claiming a substantial reduction through contractual procurement: its market-based figure (50 ktCO₂e) reflects RECs, PPAs, or green tariffs, while the location-based figure (800 ktCO₂e) reflects the physical grid it actually draws on. The question is whether that procurement is high quality and additional. You would want to ask: what instruments produce the reduction — direct PPAs and on-site generation (strong) or unbundled RECs from existing projects (weak, "REC washing")? Are the RECs geographically and temporally matched to consumption? Without additionality, the market-based number overstates real climate progress.

3. To assess credibility, ask: First, did the company reduce gross emissions through real operational change before offsetting, or is neutrality bought rather than achieved (order of operations)? Second, what is the scope coverage — does "carbon neutral" include Scope 3, or only Scope 1+2? Third, what kind of credits: avoidance or removal, what durability, what vintage, and which issuance standard (given the integrity problems with many 2018–2022 credits)? Fourth, are any avoided emissions being improperly netted against the inventory? "Carbon neutral" often relies on avoidance offsets, which newer standards constrain, so the credit quality and the reduction-first sequence are decisive.

4. There is no single right design, but three high-leverage features are defensible. First, mandatory near-complete Scope 3 coverage (a 95%-style rule with quantified, justified exclusions), because Scope 3 is where most emissions and most truncation live. Second, a strict reduction-first hierarchy with clear separation of reduced, avoided, and removed emissions — avoidance never netted, removals matched to the durability of the emissions — to stop counterfactuals masquerading as real cuts. Third, mandatory consistency between inventory, target, and claim accounting bases plus independent assurance, since inconsistency and unverified claims are where misleading-but-technically-true communication concentrates. Each targets a documented failure pattern rather than adding generic disclosure burden.
...
| SEC climate rule status | SEC ended defense 27 Mar 2025; formal rescission rulemaking submitted to OIRA 4 May 2026 (not yet finalized); rule stayed/unenforced | May 2026 | SEC 2025-58; SEC letter to 8th Cir. (May 2026); see Ch. 13 |
